    ...
    inner class Adapter : AbstractSingleRecyclerAdapter<Action, ActivityMainRecyclerBinding>(mBinding.recyclerView) {
        ...
        override fun onItemClickListener(vh: VH<ActivityMainRecyclerBinding>, view: View?) {
            when (getItem(vh.adapterPosition)?.id) {
                1 -> ImagePicker.create(packageName).album().maxSelect(9).compress(300).build().picker(thisActivity, callback)
                2 -> ImagePicker.create(packageName).album().maxSelect(9).cropByPercent(0.8f, 1f, true, false).compress(200).build().picker(thisActivity, callback)
                3 -> ImagePicker.create(packageName).camera().build().picker(thisActivity, callback)
                4 -> ImagePicker.create(packageName).camera().cropBySize(dp2px(300), dp2px(300)).build().picker(thisActivity, callback)
                5 -> ImagePicker.create(packageName).albumAndCamera().build().picker(thisActivity, callback)
                6 -> ImagePicker.create(packageName).albumAndCamera().cropByPercent(0.8f, 0.55f, true).compress(360).build().picker(thisActivity, callback)
            }
        }
